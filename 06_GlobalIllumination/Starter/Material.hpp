...
public:
    ...
    Material( vec3 _ambient, vec3 _diffuse, vec3 _specular, vec3 _emission, double _exponent):
                _ambient(_ambient), _diffuse(_diffuse),_specular(_specular), _emission(_emission), _exponent(_exponent), _local(1), _index_of_refraction(0){_emits = _emission.length() > 0;}

    Material( vec3 _ambient, vec3 _diffuse, vec3 _specular, vec3 _emission, double _exponent, double _local):
            _ambient(_ambient), _diffuse(_diffuse),_specular(_specular), _emission(_emission), _exponent(_exponent), _local(_local), _index_of_refraction(0){_emits = (_emission).length() > 0;}
