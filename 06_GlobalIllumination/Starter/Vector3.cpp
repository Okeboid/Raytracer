...
vec3 vec3::reflection(const vec3 &normal) const {
    auto d = *(this)-normal*(dot(*this, normal)*2);
    return d;
}
...
const double dot(const vec3& u, const vec3& v) {
...
vec3 vec3::rotateX(double a) {
    return vec3(_elements[0], _elements[1]*cos(a)-_elements[2]*sin(a), _elements[1]*sin(a)+_elements[2]*cos(a));
}

vec3 vec3::rotateY(double a) {
    return vec3(_elements[0]*cos(a)+_elements[2]*sin(a),_elements[2], _elements[2]*cos(a)-_elements[0]*sin(a));
}

vec3 vec3::rotateZ(double a) {
    return vec3(_elements[0]*cos(a)-_elements[1]*sin(a), _elements[0]*sin(a)+_elements[1]*cos(a),_elements[2]);
}